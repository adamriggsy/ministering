...
window.approvalHelper = function () {
  ...
  var functions = {
    ...
    'getActiveContainerById': function getActiveContainerById(ministerId) {
      return $('[data-ministerid="' + ministerId + '"] ' + css.household.comments);
    },
    'createComment': function createComment() {
      var form = $(css.comments.form);
      var ministerId = form.find('#ministerId').val();
    ...
    'commentCreateSuccess': function commentCreateSuccess(response) {
      if (response.saved) {
        ...
        _comments_js__WEBPACK_IMPORTED_MODULE_0__.commentsHelper.functions.buildComments(_comments_js__WEBPACK_IMPORTED_MODULE_0__.commentsHelper.functions.getDataAttr('commentTemplate'), functions.getActiveContainerById(functions.getDataAttr('currentMinisterId')), response.comments, 'No feedback provided.');
    ...
    'updateStatus': function updateStatus(btn, status) {
      ...
      var ministerId = container.data('ministerid');
      var householdId = container.data('householdid');
      $.ajax({
        ...
        success: function success(response) {
          if (response.saved) {
            $(css.household.status, container).html(response.status), console.log($(css.household.container, container));
